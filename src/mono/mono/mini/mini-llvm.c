	...
	for (ins = bb->code; ins; ins = ins->next) {
		...
		case OP_SETRET:
			...
			case LLVMArgVtypeAsScalar: {
				...
				gboolean ret_in_reg = FALSE;
				gboolean is_simd = MONO_CLASS_IS_SIMD (ctx->cfg, mono_class_from_mono_type_internal (sig->ret));
				switch (linfo->ret.storage) {
				case LLVMArgNormal: ret_in_reg = TRUE; break;
				case LLVMArgVtypeInReg: case LLVMArgVtypeAsScalar: ret_in_reg = is_simd; break;
				}
				if (ret_in_reg && (!lhs || ctx->is_dead [ins->sreg1])) {
					/*
